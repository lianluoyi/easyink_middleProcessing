...
<script>
  ...
  import { getAppIdAndCorpId, getOpenId, getTruthUrl } from '../../config/api';
  export default {
    data() {
      return {
        code: '',
    ...
    methods: {
      ...
      getAppid() {
        ...
        getAppIdAndCorpId(this.isShortCode ? shortCodePayload : formPayload)
          .then((res) => {
            const { corpId, appId, componentAppId } = res.data;
      ...
      getShortCode(url) {
        if (this.isShortCode) {
          ...
          window.sessionStorage.setItem('channelType', url.searchParams.get('channelType'));
      ...
      getCode(appid, componentAppId) {
        ...
        )}${componentAppId ? '&component_appid=' + componentAppId : ''}#wechat_redirect`;
      ...
      getUrl() {
        ...
        getTruthUrl({
          ...
          if (!url.searchParams.has('formId')) {
            window.location.replace(res.data.slice(0, 4) !== 'http' ? `http://${res.data}` : res.data);
          } else {
            uni.$u.route({
              url: 'pages/form/index',
              params: {
